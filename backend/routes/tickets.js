...
router.get("/", auth, async (req, res) => {
  try {
    const tickets = await Ticket.find({ ownerBstId: req.user.bstId }).sort({ createdAt: -1 });
...
router.post("/mint", auth, async (req, res) => {
  try {
    ...
    if (!tokenId || !eventId || !seat || !faceValue) {
      return res.status(400).json({ message: "tokenId, eventId, seat, faceValue are required" });
    }
    ...
    const tx = await contract.mint(buyer.walletAddress, tokenId, eventId, seat, Number(faceValue));
    await tx.wait();
    ...
    return res.status(500).json({ message: "Mint failed", error: error.message });
  }
});
